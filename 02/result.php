...
<body>
    <div id="word">
        ...
        <div id="word2">
            <ul>
                <li><?php
                        if($_POST["name1"] == "" || $_POST["name2"] == "" || preg_match("/^[\s　]+$/", $_POST["name1"]) || preg_match("/^[\s　]+$/", $_POST["name2"])){
                ...
                <li><?php
                        if(($_POST["num1"]) == "" || ($_POST["num2"]) == "" || ($_POST["num3"]) == "" ||
                            preg_match("/^[\s　]+$/", $_POST["num1"]) || preg_match("/^[\s　]+$/", $_POST["num2"]) || preg_match("/^[\s　]+$/", $_POST["num3"])){
                ...
                <li><?php
                        if($_POST["y_num1"] == "" || $_POST["y_num2"] == "" || $_POST["address1"] == "" ||
                            preg_match("/^[\s　]+$/", $_POST["y_num1"]) || preg_match("/^[\s　]+$/", $_POST["y_num2"]) ||
                             preg_match("/^[\s　]+$/", $_POST["address1"])){
                            echo "住所<br>未記入です<br>";
                        }else{
                ...
                ?></li>
                <li><?php
                        if($_POST["mail_1"] == "" || $_POST["mail_2"] == "" || preg_match("/^[\s　]+$/", $_POST["mail_1"]) || preg_match("/^[\s　]+$/", $_POST["mail_2"])){
        ...
        <div id=box>
            <div id="word3">
                <?php
                    if($_POST["area"] == "" || preg_match("/^[\s　]+$/", $_POST["area"])){
                        echo "記入内容がありません";
                    }else{
